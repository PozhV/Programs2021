...
public:
  AirportCounter() {
	  for(int i = 0; i < size; i++)
		  counts[i] = make_pair(static_cast<TAirport>(i), 0);
  }
  ...
  size_t Get(TAirport airport) const{
	  return counts[static_cast<int>(airport)].second;
  }
  void Insert(TAirport airport){
	  counts[static_cast<int>(airport)].second++;
  }
  void EraseOne(TAirport airport){
	  counts[static_cast<int>(airport)].second > 0 ? counts[static_cast<size_t>(airport)].second-- : counts[static_cast<size_t>(airport)].second = 0;
  }
...
private:
  ...
  array<Item, static_cast<size_t>(TAirport::Last_)> counts;
